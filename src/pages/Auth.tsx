...
import { openDesktopAuthInBrowser } from "@/lib/deepLink";
...
const Auth = () => {
  ...
  const nativeMobile = isCapacitorNative();
  const nativeApp = desktop || nativeMobile;
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === "signup") {
        ...
        // forgot
        const redirectBase =
          nativeApp || window.location.protocol === "file:"
  ...
  const google = async () => {
    ...
    try {
      if (nativeApp) {
        // В нативке (Tauri/Android) открываем системный/in-app браузер на handoff-страницу.
        // После успеха браузер редиректит в mirr://callback?code=... и приложение ловит deep-link.
        await openDesktopAuthInBrowser();
        toast.info("Завершите вход в открывшемся браузере");
        setLoading(false);
        return;
      }
        ...
        {mode !== "forgot" && (
          <>
            ...
            >
              ...
              {nativeApp ? "Войти через Google (откроется браузер)" : "Войти через Google"}
            </Button>
